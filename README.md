...
<p align="center">
  ...
  <sub>支持 Apple Silicon (M1/M2/M3/M4) + Intel（已在 M4 芯片上测试）</sub>
</p>
...
SimpleVoiceInput 是一款 macOS 菜单栏语音输入应用，按下 Fn 键开始录音，再次按 Fn 键停止录音并自动将语音转录为文本粘贴到光标位置。

**核心功能：**
- 按下 `Fn` 键开始录音，再次按 `Fn` 键停止并自动转录
- 使用 Google Gemini API 进行语音识别和文本处理
- 自动粘贴到当前应用的光标位置
- 菜单栏应用，不干扰工作流

**设置功能（General 页面）：**
- **API Key 管理**：配置和验证 Google Gemini API Key
- **录音快捷键**：设置 Fn 键作为录音触发键
- **麦克风选择**：选择系统默认麦克风或指定音频输入设备
- **系统提示词**：自定义 Gemini AI 的处理指令，优化转录效果
- **权限管理**：一键授予麦克风和辅助功能权限

**历史记录功能（History 页面）：**
- **录音历史**：本地存储所有录音记录，保护隐私
- **播放回放**：内置音频播放器，可随时回听录音
- **转录结果**：查看每次的转录文本，支持一键复制
- **处理详情**：显示使用的模型和提示词，便于调试
- **文件管理**：快速定位原始音频文件
...
3. **按下 `Fn` 键**开始录音
4. 对着麦克风说话
5. **再次按 `Fn` 键**，应用会自动转录并粘贴文本

## 系统要求

- macOS 13.0 或更高版本
- Apple Silicon (M1/M2/M3/M4) 或 Intel 处理器（已在 M4 芯片上测试）
