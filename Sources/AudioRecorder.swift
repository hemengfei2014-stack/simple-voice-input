...
class AudioRecorder: NSObject, ObservableObject {
    ...
    private let audioFileQueue = DispatchQueue(label: "com.simplevoiceinput.app.audiofile")
                ...
                self.audioFileQueue.sync {
                    if let file = self.audioFile {
                        do {
                            ...
                            os_log(.error, log: recordingLog, "Failed to write audio buffer: %@", error.localizedDescription)
                            self.audioFile = nil
                        }
                    }
                }
